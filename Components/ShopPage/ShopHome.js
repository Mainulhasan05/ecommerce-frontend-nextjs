...
const ShopHome = ({ data }) => {
    ...
    const handleSelect = async (e) => {
        ...
        router.replace({
            pathname: '/shop/'+router.query.slug,
            query: {
                page: e.selected + 1,
                limit: limit,
                price: price
        ...
        const asPath= `/api/shop/${router.query.slug}?page=${e.selected + 1}&limit=${limit}&price=${price}`
        setLoading(true)
        const res=await axiosInstance.get(`${asPath}`)
        setLoading(false)
